Sets
 ...
 k            Index Stützstellen für Erweiterung Veluste                           /k0*k10/
...
Scalars
 ...
 alpha        Reibungskoeffizient [s pro m^2]                                         /0.004804688/
;
...
Parameters
 ...
 Q_Stuetz_Verlust (k)  Stützstellen (verschiedene Durchflüsse) für die Verluste [m3 pro s]
 P_Stuetz_Verlust (k)  Zugehörige Leistungen an den Stützstellen für die Verluste [MW]
 ;
...
* Äquidistante Stützstellen von QVerlust und zugehörige Leistungen PVerlust
 Q_Stuetz_Verlust(k)                    = (ord(k)-1)/(card(k)-1)*Q_max;
*In Zeile 73: Müsste hier nicht noch v_on_Turb(t) multipliziert werden, um die Turbinenflüsse beider Maschinensätze zu berücksichtigen?
 P_Stuetz_Verlust(k)                    =  (Q_Stuetz_Verlust(k)**3)*9.81/1000*alpha;
...
SOS2 variables
 ...
 v_lambda_Turbinenverluste(t,k) SOS2-Variablen zur linearen Approximation der Turbinenverluste 
;
Integer variables
...
Equations
 ...
 g_Turbinen_SummeLambda_Verlust (t) Summe Gewichte der Verlust-Approximation entspricht Anzahl Maschinensätze im Turbinenbetrieb
...
* Leistungen und VDurchflüsse im Turbinenbetrieb
 g_Turbinen_Leistung(t)       .. v_P_Turb(t)  =e= sum(n,v_lambda_Turbine(t,n) * P_Stuetz(n)) - sum(k,v_lambda_Turbinenverluste(t,k) * P_Stuetz_Verlust(k));
 ...
 g_Turbinen_SummeLambda_Verlust(t)    .. v_on_Turb(t) =e= sum(k,v_lambda_Turbinenverluste(t,k));
...
execute_unload "results.gdx" v_P_Turb.L v_P_Pump.L v_Q_Turb.L v_Q_Pump.L v_Inh.L v_Erloes.L v_on_Turb
...
*execute 'gdxxrw.exe  results.gdx o=Input-Output_Kurzfristig.xlsx squeeze=N var=v_on_Turb.L  rng=Output!K3 rdim=1 cdim=0'
