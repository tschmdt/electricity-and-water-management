...
Sets
 ...
 k            Index Stützstellen für Triebwasserweg Durchfluss im Turbinenbetreib  /k0*k20/
...
Scalars
 ...
 a            Reibungskoeffizient                                                     /0.004804688/
;
...
Parameters
 ...
 Q_Stuetzverlust(k) Stützstellen (verschiedene Durchflüsse) für Verluste [m3 pro s]
 P_Stuetzverlust(k) Zugehörige Leistungen an den Stützstellen für die Verluste [MW]
 ;
...
* Äquidistante Stützstellen von Q und zugehörige Verlustleistungen
 Q_Stuetzverlust(k)                            = (ord(k)-1)/(card(k)-1)*Q_max; #Teilt in gleichmäßigen Abständen die Q's ein
 P_Stuetzverlust(k)  =  (Q_Stuetzverlust(k)**3)*9.81/1000*a;
# P_Stuetzverlust(k)$(Q_Stuetzverlust(k)/Q_max>0.095)  =  (Q_Stuetzverlust(k)**3)*9.81/1000*a;
# P_Stuetz(n)$(Q_Stuetz(n)/Q_max<0.095)  =  0;
...
Positive variables 
 ...
 v_P_Turbverlust  (t  )  Verlustleistung im Turbinenbetrieb des Kraftwerkes in der Stunde von t-1 bis t [MW]
 v_Q_Turbverlust  (t  )  Verlorene Turbinendurchflüsse der Maschinensätze des Kraftwerkes in der Stunde von t-1 bis t [m3 pro s]

;
SOS2 variables
 v_lambda_Turbine (t,n)  SOS2-Variablen zur linearen Approximation der Leistungs-Kennlinie im Turbinenbetrieb  [1]
 v_lambda_Verlust (t,k)  SOS2-Variablen zur linearen Approximation der Verlust-Leistung im Turbinenbetrieb
;
Integer variables
...
Equations
 ...
 g_Verlustleistung         (t)    Verlustleistung im Turbinenbetrieb
 g_Verlustdurchfluss       (t)    Verlorener Durchfluss durch Verluste
 g_Verlust_SummeLambda     (t)    Summe Gewichte
 g_1Verlustdurchfluss(t)
;

* Zielfunktion
 g_Erloes                     .. v_Erloes =e= sum(t,(Preis(t)-12)*(v_P_Turb(t)-v_P_Turbverlust(t))-(Preis(t)+20)*v_P_Pump(t));

* Speicherinhalt
# g_BetriebsinhaltSpeicher(t)  .. (v_Inh(t)-v_Inh(t-1)-I_Start$t_start(t))*1000000 =e= 3600*(Dargebot-v_Q_Turb(t)-v_Q_Turbverlust(t)+v_Q_Pump(t));
...
* Verlustleistung im Turbinenbetrieb
g_Verlustleistung(t)          .. v_P_Turbverlust(t) =e= sum(k,v_lambda_Verlust(t,k) * P_Stuetzverlust(k));
g_Verlustdurchfluss(t)        ..  v_Q_Turbverlust(t) =e=  sum(k,v_lambda_Verlust(t,k) * Q_Stuetzverlust(k));
g_1Verlustdurchfluss(t)        ..  v_Q_Turb(t) =e=  v_Q_Turbverlust(t); #zusammenhang durchfluss turbine hängt ab von azahl maschinensätze und
# summierter trubinendurchfluss beider MS = durchfluss des triebwasserweges
g_Verlust_SummeLambda(t)      .. 1 =e= sum(k,v_lambda_Verlust(t,k)); #summe =1 weil 1 triebwasserweg
...
put 't':12:3,'Anz. Masch':12:3/
loop(t,put , ord(t):12:3, v_on_Turb.l(t):12:3 /);
