...
*Pruefung 3
*Pv_Pump       Verluste der Pumpe als Parameter. Verluste sind konstant hängt nur davon ab ob 1 oder 2 Maschinen im Einsatz
 Q_vpStuetz(n)
 P_vpStuetz(n)
;
...
*Pruefung 3
* Verluste bei der Energiewandlung im Pumpbetrieb
* Brauchen keine SO2 Variablen, weil der Durchfluss der Pumpe konstant ist mit Q = 35m^3/s
* Parameter der Pumpverluste definieren und dann unten Gleichung verwenden, um Nicht-Linearität aus dem System zu bekommen
* Pv_Pump = 9.81/1000*alpha*Q_Pump**3;
 Q_vpStuetz(n)                                = (ord(n)-1)/(card(n)-1)*(2*Q_pump);
 P_vpStuetz(n)$(Q_vpStuetz(n)/(2*Q_pump)>0.095) = 9.81/1000*alpha*Q_vpStuetz(n)**3;
 P_vpStuetz(n)$(Q_vpStuetz(n)/(2*Q_pump)<0.095)  =  0;
...
*Pruefung 3
 v_Pv_Pump        (t  )  Verlsutleistung des Kraftwerks im Pumpbetrieb in MW
;
SOS2 variables
...
*Pruefung 3
g_Verlust_Pumpe           (t)    Verlustleistung der Pumpe Aufgabenstellung 3
g_Verlust_Pumpe_Summenlambda (t)
g_Durchfluss_TB           (t)
;
...
*Pruefung 2&3
g_Profit                     .. v_Erloes =e= sum(t,(Preis(t)-12)*(v_P_Turb(t)-v_Pv_Turb(t))-(Preis(t)+20)*(v_P_Pump(t)+v_Pv_Pump(t)));
...
*Pruefung 3
* Verlustleistung der Pumpe
g_Verlust_Pumpe(t)           .. v_Pv_Pump(t) =e= sum(n,v_lambda_Verlust_pump(t,n)*P_vpStuetz(n));
g_Durchfluss_TB(t)           .. v_Q_Pump(t)  =e= sum(n,v_lambda_Verlust_pump(t,n)*Q_vpStuetz(n));
g_Verlust_Pumpe_Summenlambda (t) .. sum(n,v_lambda_Verlust_pump(t,n)) =e= 1;
