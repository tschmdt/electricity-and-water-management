...
Scalars
 ...
 Peg_approx         Pegelstand für das Bisektionsverfahren [m]
...
Parameters
 ...
 H_alt(t)             Fallhöhen aus der vorangegangenen Iteration [m]
...
;
 ...
 e_Erloes           .. v_Erloes                 =e= sum(t,(p_Preis(t)-16)*v_P_Turb(t));
 e_Fallhoehe(t)     .. v_Fallhoehe(t)           =e= H_alt(t)+Steigung_alt(t)*(v_EnInh(t)-E_alt(t));
...
*Zusatz:
e_MaxRampDown(t)$(ord(t)>1) .. v_P_Turb(t-1)-v_P_Turb(t)=l=100;

Model Energiebilanzmodell /all/
...
**** Fallhöhe der i-ten Iteration (Ermittlung mittels Bisektionsverfahren)
     ...
     Loop(j,
         Peg_approx = (Peg_Untergrenze+Peg_Obergrenze)/2;
         E_approx = 9.81/3.6*sum(k,Koeff(k)*(Peg_approx-Peg_min)**ord(k));
         if(E_approx>E_alt(t), Peg_Obergrenze=Peg_approx);
         if(E_approx<E_alt(t), Peg_Untergrenze=Peg_approx);
     );
     H_alt(t) = Peg_approx-H_Bezug;
**** Ableitung der Fallhöhe nach dem Energieinhalt an der Stelle des optimalen Energieinhaltes der i-ten Iteration (Ermittlung mittels Umkehrregel)
     Steigung_alt(t) = 1/(9.81/3.6*sum(k,ord(k)*Koeff(k)*(Peg_approx-Peg_min)**(ord(k)-1)));
  );
* II. Lösen des linearen Problems:
 solve Energiebilanzmodell using LP maximizing v_Erloes;
 );

****************************

execute_unload "results2.gdx" v_EnInh.L e_Energieinhalt.M v_P_Turb.L
